...
{
    $jenis = $request->jenis ?? null; // default null, bukan 'barang'

    $data = collect(); // kosong secara default
    $jurusanList = collect(); // kosong juga

    if ($jenis) { // hanya query jika ada jenis yang dipilih
        $bulan   = $request->bulan;
        $tahun   = $request->tahun;
